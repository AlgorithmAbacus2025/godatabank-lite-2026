...
if not valid_records:
    raise ValueError("No valid GDP per capita values found.")

latest = valid_records[-1]
highest = max(valid_records, key=lambda item: item["value_usd"])
lowest = min(valid_records, key=lambda item: item["value_usd"])
...
analysis = {
    ...
    "highest_value_usd": highest["value_usd"],
    "lowest_year": lowest["year"],
    "lowest_value_usd": lowest["value_usd"],
...
print(f"Latest valid value: ${analysis['latest_value_usd']} in {analysis['latest_valid_year']}")
print(f"Highest value: ${analysis['highest_value_usd']} in {analysis['highest_year']}")
print(f"Lowest value: ${analysis['lowest_value_usd']} in {analysis['lowest_year']}")
